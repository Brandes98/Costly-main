...
export default function Spinner({ size = 'md' }) {
  ...
  return (
    <div className={`${s} border-2 border-border border-t-tl rounded-full animate-spin`} />
  )
}
...
export function KpiCard({ icon, value, label, delta, deltaUp, variant = 'tl' }) {
  ...
  return (
    <div className={`kpi relative after:content-[''] after:absolute after:top-0 after:right-0 after:w-12 after:h-12 after:rounded-bl-full after:opacity-[0.08] ${colors.after}`}>
      <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm mb-2.5 ${colors.icon}`}>
        {icon}
      </div>
      <div className="text-2xl font-bold text-ink leading-none">{value}</div>
      <div className="text-[11px] text-mist mt-0.5">{label}</div>
      {delta && (
        <div className={`text-[10.5px] font-medium mt-2 ${deltaUp ? 'text-sg' : 'text-rs'}`}>
          {deltaUp ? '↑' : '↓'} {delta}
        </div>
      )}
    </div>
