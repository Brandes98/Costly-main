...
import Spinner, { EmptyState, Modal } from '../../components/ui/Spinner';
...
export default function ImportacionesPage() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useImportaciones({ estado: estado || undefined });
  const { data: pedidosSinImp = [], isLoading: loadingPedidos } = usePedidos({
    sin_importacion: 'true',
  });
                  ...
                  <div className="flex flex-wrap gap-1">
                    {item.proveedores.length > 0 ? (
                      item.proveedores.map((p) => (
                        <span key={`${item.importacion_id}-${p}`} className="ic">
                          {p}
                        </span>
                      ))
                    ) : (
      ...
      >
        <div className="space-y-4">
          ...
          <div className="rounded-card border border-tl/20 bg-tl-xl px-3 py-2 text-xs text-slate">
            Un solo pedido → <strong className="text-tl">importación individual</strong>. Dos o
            más → <strong className="text-tl">importación consolidada</strong>.
          </div>
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <div className="text-[10px] text-mist">
                            {p.proveedor?.nombre || '—'} ·{' '}
                            {p._count?.lineas ?? p.lineas?.length ?? 0} líneas
                          </div>
                        </div>
                      </div>
                      <span
                        className={`pill ${p.estado === 'confirmado' ? 'pill-green' : 'pill-yellow'}`}
                      >
                        {p.estado}
                      </span>
                    </div>
