...
export default function DashboardPage() {
  ...
  const transito  = pedidos.filter(p => ['embarcado','en_transito','en_puerto_cr'].includes(p.estado)).length
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        <KpiCard icon="📦" value={activos}  label="Pedidos activos"   delta="Total en proceso" deltaUp variant="tl" />
        <KpiCard icon="🚢" value={transito} label="En tránsito"       delta="En camino a CR"   deltaUp variant="gd" />
        <KpiCard icon="⏳" value={alertas}  label="Alertas vencidas"  delta="Requieren atención"       variant="rs" />
        <KpiCard icon="🏛" value={enAduana} label="En aduana/fiscal"  delta="Trámite pendiente"        variant="sg" />
      </div>
      ...
      {tc && (
        <div className="bg-tl-xl border border-tl/20 rounded-card px-4 py-2.5 flex items-center gap-3 text-xs">
          ...
          <span className="font-bold text-ink">₡{Number(tc.usd_crc).toLocaleString('es-CR', { minimumFractionDigits: 2 })} / USD</span>
        ...
        <div className="card">
          <div className="card-header">
            ...
            <div className="flex gap-3 text-[10.5px] text-mist">
              <span className="flex items-center gap-1"><span className="s3 s3g" />OK</span>
              <span className="flex items-center gap-1"><span className="s3 s3y" />Pronto</span>
              <span className="flex items-center gap-1"><span className="s3 s3r" />Vencido</span>
            </div>
          </div>
          {pedidos.length === 0 ? (
            ...
            <table className="tbl">
              ...
              <tbody>
                {pedidos.slice(0, 8).map(p => {
                  ...
                  return (
                    <tr key={p.pedido_id} className="cursor-pointer" onClick={() => window.location.href = `/pedidos/${p.pedido_id}`}>
                      <td className="pl-3">
                      ...
                      <td><span className="pill pill-gray">{p._count?.lineas} líneas</span></td>
                    </tr>
                  )
                })}
              </tbody>
          ...
          <div className="card">
            ...
            <div className="card-body p-0">
              {hitos.slice(0, 5).map(h => {
                ...
                return (
                  <div key={h.hito_id} className="flex gap-2 items-start px-4 py-2.5 border-b border-border-lt last:border-b-0">
                    <div className={`w-7 h-7 rounded-md flex items-center justify-center text-xs flex-shrink-0 ${sem === 'red' ? 'bg-rs-l' : 'bg-gd-l'}`}>
                      {sem === 'red' ? '🚨' : '⏰'}
                    </div>
