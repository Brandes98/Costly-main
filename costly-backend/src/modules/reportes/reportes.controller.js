...
export const getAll = async (req, res) => {
  try {
    return successResponse(res, await service.getAll(req.user.empresa_id, req.user.usuario_id, req.query))
...
export const generar = async (req, res) => {
  try {
    return successResponse(res, await service.generar(req.user.empresa_id, req.body))
  } catch (error) { return errorResponse(res, error) }
}
 
export const save = async (req, res) => {
  try {
    return successResponse(res, await service.save(req.user.empresa_id, req.user.usuario_id, req.body), 201)
  } catch (error) { return errorResponse(res, error) }
}
